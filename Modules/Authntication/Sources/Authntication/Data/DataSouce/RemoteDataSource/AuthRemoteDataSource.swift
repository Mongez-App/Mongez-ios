...
public protocol AuthRemoteDataSourceProtocol {
    func handshake(idToken: String, name: String, avatarUrl: String) async throws -> (dto: AuthResponseDTO, statusCode: Int)
...
public class AuthRemoteDataSource: AuthRemoteDataSourceProtocol {
    ...
    public func handshake(idToken: String, name: String, avatarUrl: String) async throws -> (dto: AuthResponseDTO, statusCode: Int) {
        let result = try await NetworkManger.shared.requestWithStatus(
            endpoint: AuthEndpoint.handshake(idToken: idToken, name: name, avatarUrl: avatarUrl),
