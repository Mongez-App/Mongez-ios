...
public class AuthRemoteDataSource: AuthRemoteDataSourceProtocol {
    ...
    public func login(email: String, password: String) async throws -> AuthResponseDTO {
        try await NetworkManager.shared.request(endpoint: AuthEndpoint.login(email: email, password: password), responseType: AuthResponseDTO.self)
    }
    public func register(name: String, email: String, password: String) async throws -> AuthResponseDTO {
        try await NetworkManager.shared.request(endpoint: AuthEndpoint.register(name: name, email: email, password: password), responseType: AuthResponseDTO.self)
    }
}
